 ...
 */
export function countChangedLines(stdout: string): { additions: number; deletions: number } {
  let additions = 0
  let deletions = 0
 ...
 * @param stdout - raw `git diff` text for one path.
 */
export function parseUnifiedDiff(stdout: string): GitDiffLine[] {
  const lines: GitDiffLine[] = []
  for (const raw of stdout.replace(/\r\n/g, '\n').split('\n')) {
    if (raw.length === 0 && lines.length === 0) continue
    if (
    ...
    if (raw.length > 0) lines.push({ kind: 'meta', text: raw })
  }
  return lines
}
