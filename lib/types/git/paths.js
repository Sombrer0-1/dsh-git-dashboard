 ...
 */
export function isUnsafeRepositoryPath(candidate) {
    ...
    if (candidate.startsWith('/'))
        return true;
    if (MAGIC_PATHSPEC.test(candidate))
        return true;
 ...
 */
export function normalizePosixRelative(relative) {
    ...
    for (const part of parts) {
        if (part === '' || part === '.')
            continue;
        if (part === '..')
            throw new Error('path escapes repository root with ..');
        out.push(part);
    }
    return out.join('/');
}
 ...
 */
export function scopePrefix(repoRoot, workspacePath) {
 ...
 */
export function statusScopeArgs(prefix) {
    if (prefix === '')
        return [];
    return ['--', prefix];
}
 ...
 */
export function assertSafeCompareRef(ref) {
    if (ref.trim().length === 0)
        throw new Error('compare ref must be non-empty');
    if (ref.includes('\0'))
        throw new Error('compare ref must not contain NUL');
    if (ref.startsWith('-'))
        throw new Error('compare ref must not look like an option');
}
/**
 ...
 */
export function assertScopedFilePath(candidate, prefix) {
    if (isUnsafeRepositoryPath(candidate))
